...
void multiply_by_A_transpose_approx(poly_matrix y, poly_matrix A, poly_matrix x)
	{
	// y[0,...,(d-1)] <- x
	memcpy(y, x, PARAM_N * PARAM_D * sizeof(scalar));
	
	// y[d, ..., m'] <- x^T * A
	poly_matrix y_d = poly_matrix_element(y, PARAM_Mprime, 0, PARAM_D);
	mul_crt_poly_matrix(y_d, x, A, 1, PARAM_D, PARAM_Mprime - PARAM_D, LOG_R);
...
void construct_A_m(poly_matrix A, scalar *m)
	...
	scalar mgT_coeffs[(PARAM_K - PARAM_L)* PARAM_N];
	...
	for(int i = 0; i < (PARAM_K - PARAM_L); ++i)
		...
		for(int j = 0 ; j < SMALL_DEGREE ; ++j)
			{
			mgT_i[j] = reduce_double_naive( ((double_scalar) pow(PARAM_B, PARAM_L + i)) * m[j]);
			}
	...
	for(int i = 0 ; i < PARAM_D ; ++i)
		...
		poly A_idpluski = poly_matrix_element(A, PARAM_Mprime - PARAM_D, i, PARAM_D + i * (PARAM_K - PARAM_L));
		

		
		add_poly(A_idpluski, A_idpluski, mgT, (PARAM_K - PARAM_L) * PARAM_N - 1);
		}
